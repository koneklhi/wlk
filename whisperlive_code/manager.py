...
Original file is located at
    https://colab.research.google.com/drive/1fhgvKa0ouggypE1EKLIufQ7A6Dnvk8SN
"""
...
class WordCorrectionManager:
    ...
    def _load_base_from_json(self) -> dict:
        ...
        if not isinstance(data,list):
            print(f"[Warning] JSON root element should be a list")
            return replacements

        for entry in data:
            if not isinstance(entry,dict):
                print(f"[Warning] eACH ITEM IN json MUST BE an object")
                continue
    ...
    def refresh_replacements(self):
        ...
        # 긴 단어부터 대치하도록 정렬 (Sorting by length descending)
        # 예: "국방부"가 "국방"보다 앞에 오도록 함
    ...
    def add_user_word(self, wrong, correct):
        with sqlite3.connect(self.db_path) as conn:
            cursor = conn.cursor()
            cursor.execute("INSERT OR REPLACE INTO user_corrections (wrong_word, correct_word) VALUES (?, ?)", (wrong, correct))
            conn.commit()
        self.refresh_replacements()
