...
Original file is located at
    https://colab.research.google.com/drive/1vwijGJE8lkvFNsB7C515JTnZzeATUVMH
"""
...
class Segment:
    ...
    timestamp_forced_completion: Optional[bool]
...
class BatchedInferencePipeline:
    ...
    def forward(self, features, tokenizer, chunks_metadata, options):
        ...
        segmented_outputs = []
        segment_sizes = []
        for chunk_metadata, hypos in zip(chunks_metadata, outputs):
            ...
            for hypo in hypos:
                ...
                segments_for_this_hypo = []
                for sub in subsegments:
                    segments_for_this_hypo.append(
                        {
                            "text": tokenizer.decode(sub["tokens"]),
                            "avg_logprob": hypo["avg_logprob"],
                            "no_speech_prob": hypo["no_speech_prob"],
                            "tokens": sub["tokens"],
                            "start": sub["start"],
                            "end": sub["end"],
                            "compression_ratio": get_compression_ratio(
                                tokenizer.decode(sub["tokens"])
                            ),
                            "seek": int(
                ...
                chunk_candidates.append(segments_for_this_hypo)

            segmented_outputs.append(chunk_candidates)
    ...
    def generate_segment_batched(
        ...
        if options.multilingual:
            ...
            language_token_index = prompt.index(tokenizer.language_token)

            for i, language_token in enumerate(language_tokens):
                prompts[i][language_token_index] = language_token

        # ask for 'best_of' hypos per input in the batch
        ctr_results = self.model.model.generate(
            encoder_output,
            prompts,
        ...
        output = []
        for result in ctr_results:
            hypos = []
            for tokens, score in zip(result.sequences_ids, result.scores):
                text = tokenizer.decode(tokens)
                avg_lp = score / (len(tokens) + 1)
                hypos.append(
                    {
                        "text": text,
                        "tokens": tokens,
    ...
    def transcribe(
        ...
        no_repeat_ngram_size: int = 2,  # 0
        temperature: Union[float, List[float], Tuple[float, ...]] = [0.0, 0.2, 0.4],
        compression_ratio_threshold: Optional[float] = 2.0,
        log_prob_threshold: Optional[float] = -0.5, # -1.0
        no_speech_threshold: Optional[float] = 0.7, # 0.6
        ...
        Arguments:
            ...
            temperature: Temperature for sampling. If a list or tuple is passed, only the first value is used.
            initial_prompt: Optional text string or iterable of token ids to provide as a prompt for the each window.
            suppress_blank: Suppress blank outputs at the beginning of the sampling.
            suppress_tokens: List of token IDs to suppress. -1 will suppress a default set of symbols as defined in `tokenizer.non_speech_tokens()`.
            without_timestamps: Only sample text tokens.
            word_timestamps: Extract word-level timestamps using the cross-attention pattern and dynamic time warping, and include the timestamps for each word in each segment. Set as False.
            ...
            vad_filter: Enable the voice activity detection (VAD) to filter out parts of the audio without speech. This step is using the Silero VAD model https://github.com/snakers4/silero-vad.
            ...
            clip_timestamps: Optionally provide list of dictionaries each containing "start" and "end" keys that specify the start and end of the voiced region within `chunk_length` boundary. vad_filter will be ignored if clip_timestamps is used.
        ...
        Unused Arguments
            ...
            log_prob_threshold: If the average log probability over sampled tokens is below this value, treat as failed.
            no_speech_threshold: If the no_speech probability is higher than this value AND the average log probability over sampled tokens is below `log_prob_threshold`, consider the segment as silent.
            condition_on_previous_text: If True, the previous output of the model is provided as a prompt for the next window; disabling may make the text inconsistent across windows, but the model becomes less prone to getting stuck in a failure loop,
            such as repetition looping or timestamps going out of sync. Set as False
            prompt_reset_on_temperature: Resets prompt if temperature is above this value. Arg has effect only if condition_on_previous_text is True. Set at 0.5
            ...
            hallucination_silence_threshold: Optional[float]
                When word_timestamps is True, skip silent periods longer than this threshold (in seconds) when a possible hallucination is detected. set as None.
        ...
        if not clip_timestamps:
            if vad_filter:
                ...
                clip_timestamps = active_segments
        ...
        print("clip_timestamps: ", clip_timestamps)
        duration_after_vad = (
    ...
    def _batched_segments_generator(
        ...
        for i in range(0, len(features), batch_size):
            ...
            for hypos in results:
                # hypos is List[Dict] of up to best_of candidates
                yield hypos
...
class WhisperModel:
    def __init__(
        ...
        **model_kwargs,
        ...
        Args:
            ...
            files: Load model files from the memory.
                This argument is a dictionary mapping file names to file contents as file-like or bytes
                objects. If this is set, model_path acts as an identifier for this model.
        """
        ...
        self.last_timestamp_rotation_queue_index = 0
        self.last_timestamp_rotation_queue = [-1] * self.last_timestamp_rotation_queue_length
        # initialize with -1; store the last_timestamp of decoding result (rotation_queue는 개발 편의를 위해서...)

        self.last_timestamp_diff_threshold = 0.1 # if difference of minimum, maximum value of last_timestamp_rotation_queue is below this threshold -> trigger the split event (kinda blocked?)
    ...
    # 여기에서 parameter 조절
    def transcribe(  # 여기에서 whisper param 수정!!!!!!@@!!!
        self,
        audio: Union[str, BinaryIO, np.ndarray],
        language: Optional[str] = None,
        task: str = "transcribe",
        log_progress: bool = False,
        beam_size: int = 7,
        best_of: int = 7,  # 5 -> 7 decoder에서 결정하는 후보군 개수...
        patience: float = 1,
        length_penalty: float = 1,
        repetition_penalty: float = 1.3,
        no_repeat_ngram_size: int = 2,  # 2 -> 5
        ...
        compression_ratio_threshold: Optional[float] = 2.2,
        log_prob_threshold: Optional[float] = -1.0,
        no_speech_threshold: Optional[float] = 0.9,  # 0.75 -> 0.9
        condition_on_previous_text: bool = False,
        prompt_reset_on_temperature: float = 0.8,
        initial_prompt: Optional[Union[str, Iterable[int]]] = None,
        prefix: Optional[str] = None,
        suppress_blank: bool = True,
        suppress_tokens: Optional[List[int]] = [-1],
        ...
        hallucination_silence_threshold: Optional[float] = None, # 0.6 -> 0/8
        hotwords: Optional[str] = None,
        language_detection_threshold: Optional[float] = 0.7,
        language_detection_segments: int = 1,
    ) -> Tuple[Iterable[Segment], TranscriptionInfo]:
        ...
        Arguments:
            ...
            clip_timestamps: Comma-separated list start,end,start,end,... timestamps (in seconds) of
                clips to process. The last end timestamp defaults to the end of the file.
                vad_filter will be ignored if clip_timestamps is used.
            hallucination_silence_threshold: When word_timestamps is True, skip
                silent periods longer than this threshold (in seconds) when a possible
        ...
        if vad_filter and clip_timestamps == "0":
            ...
            print(f"\n[Transcriber.py]: VAD filter removed {format_timestamp(duration - duration_after_vad)} of audio")
        ...
        print(f"transcribe input Language: {language}")
        ...
        if language is None:
            if not self.model.is_multilingual:
                ...
                last_timestamp = self.last_timestamp_rotation_queue[self.last_timestamp_rotation_queue_index % self.last_timestamp_rotation]
                seek_end = (
                    int(last_timestamp * self.frames_per_second)
                    if last_timestamp >0 and last_timestamp * self.frames_per_second < content_frames
                    else None
                )
                ...
                (
                    ...
                    features=features[..., seek:seek_end],
                    language_detection_segments=language_detection_segments,
                    language_detection_threshold=language_detection_threshold,
                )
        ...
        print(f"Tokenizer Language: {language}")
    ...
    def _split_segments_by_timestamps(
        ...
        # =========================================================================
        # print("\n" + "=" * 60)
        # print(f"[DEBUG] Analyzing Chunk at time offset: {time_offset:.2f}s")
        # debug_sequence = []
        # for t in tokens:
        #     if t >= tokenizer.timestamp_begin:
        #         time_val = (t - tokenizer.timestamp_begin) * self.time_precision
        #         debug_sequence.append(f"<|{time_val:.2f}|>")
        #     else:
        #         debug_sequence.append(tokenizer.decode([t]))
        # print(f"[DEBUG] Raw Token IDs (length {len(tokens)}):")
        # print(tokens)
        # print("\n[DEBUG] Interleaved Text and Timestamps:")
        # print("".join(debug_sequence))
        # print("=" * 60 + "\n")
        # =========================================================================
        # DEBUG BLOCK END
        # =========================================================================

        # rotation_queue update
        last_timestamp = -1
        if tokens[-1] >= tokenizer.timestamp_begin:
            time_val = (tokens[-1] - tokenizer.timestamp_begin) * self.time_precision
            last_timestamp = time_val

        # discrminate forced_split event
        timestamp_forced_completion = False

        self.last_timestamp_rotation_queue_index += 1
        self.last_timestamp_rotation_queue[self.last_timestamp_rotation_queue_index % self.last_timestamp_rotation_queue_length] = last_timestamp
        print(f"Timestamp Rotation Queue: {self.last_timestamp_rotation_queue}")

        if -1 not in self.last_timestamp_rotation_queue:
            if max(self.last_timestamp_rotation_queue) - min(self.last_timestamp_rotation_queue) < self.last_timestamp_diff_threshold:
        ...
        if len(consecutive_timestamps) > 0:
            ...
            current_segments.append(
                dict(
                    ...
                    tokens=tokens,
                    timestamp_forced_completion=timestamp_forced_completion
                )
            )
    ...
    def generate_segments(
        ...
        while clip_idx < len(seek_clips):
            ...
            for segment in current_segments:
                tokens = segment["tokens"]
                text = tokenizer.decode(tokens)
                timestamp_forced_completion = segment["timestamp_forced_completion"]
    ...
    def detect_language(
        ...
        for i in range(0, features.shape[-1], self.feature_extractor.nb_max_frames):
            ...
            target_langs_list = self.target_langs
            target_langs_set = set(target_langs_list)

            filtered_probs = [(lang, prob) for lang, prob in all_language_probs if lang in target_langs]
            if filtered_probs:
                # en, ko 중 가장 높은 것 선택
                language, language_probability = max(filtered_probs, key=lambda x: x[1])
                # 💡 [안전장치 추가]
                # 만약 en, ko 중 1등을 한 확률조차 10% (0.1) 미만이라면?
                # -> 이건 잡음을 억지로 쥐어짜는 상황이므로 확률을 0으로 만들어버림
                if language_probability < 0.1:
                    language_probability = 0.0
            else:
                language, language_probability = target_lang_list[0], 0.0
