...
4. **문장 분리 로직**: 전사 txt의 `[문장별 확정 트리거]` 섹션(각 문장 뒤 `⟨silence/punctuation/language_switch/speaker_change⟩`)을 읽어, 문장이 **어떤 로직으로 확정·분리됐는지** 분석한다. 조기/지연 확정, 코드스위칭·화자전환 경계에서 트리거가 기대대로 작동했는지 F1 정성 판단에 활용한다.
