...
- **목표 필수 기능 예외**: §3.1·§3.2 불변 제약 달성에 필요한 기반 기능이 정량 게이트에서 탈락해도 Claude는 자율 기각하지 않고 사용자에게 채택 여부를 질의한다. **검토자(antigravity)는 이 범주의 기능에 대해 단독 기각 권고를 내리지 않는다** — 대신 트레이드오프(회귀 위치·대안 구현)를 분석해 사람이 결정하도록 지원한다.
