...
|-----|-------|------|------|--------------|-------------|-------------|------|
| Exp-176 | **E5** | 2026-07-12 | 문법-조건부 침묵 경계 게이트 — Case B(단어중간분절) 방지, `SILENCE_HARD_SECS=0.8` 안전망+decide-late+이원유예+메모+hard_boundary스탬프(`tokens_alignment.py`), 워크트리 브랜치 대기(미머지) | 29.3%(max38.7, OFF28.7/36.9) | 16.7%(max29.6🟡, OFF19.7/20.2) | **11.9%**(max13.1, OFF20.2/30.4 대폭개선) | 🟡 채택 권고(사용자 승인 대기, master 미머지) — 화자분리F1 3파일 전부 worst-case 개선(sbs1 50%→100%), Case B 게이트 통제범위 N=3 전회차 재발 0건, held-out ytn1 10.4%(21.5%→대폭개선)·eng1 3.8% 양호, 영어대문자 규칙 회귀없음; ytn2 WER max만 OFF 대비 +9.4pt(참고게이트 34.5%는 이내, 원인은 토큰반복 환각 추정이나 경계로직 무관 여부 미확증) — 사용자 질의 대기 |
