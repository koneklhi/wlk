...
## Exp-212 — 환각 감소 파라미터 스윕(CRT·logprob) — 노이즈 바닥 N=9 실측 + QualityGate 이중역할 규명 (2026-08-18) [E7, 전 후보 보류/기각, 코드 무변경]

**측정 언어모드**: `auto` (전 설정·전 파일)

### 발단
사용자 지시(2026-08-18): "`BOUNDARY_PROTECT_SECS`는 5초로 유지한 채, VAD 등 파라미터 조정으로 환각을
줄여봐라. 2시간 자율 진행." → 코드 변경 없이 **CLI 노출 파라미터만** 스윕하는 실험으로 설계.
`parse_args.py` 기준 노출 레버를 조사해 후보를 좁혔다.

### 가설
Exp-210이 규명한 "QG streak → `refresh_segment` 폐기가 유실과 환각을 함께 만든다"는 사실에 근거해,
QualityGate 임계값(logprob·compression-ratio)을 조정하면 환각 발생률을 바꿀 수 있다. 방향은 양쪽 다
가능하다 — **조이면** 환각 텍스트가 직접 억제되고, **풀면** streak 도달이 줄어 폐기(=환각의 원인)가 준다.

### 변경 내용
**코드 변경 없음.** 서버 CLI 플래그만 변경(`scripts/eval.py` 패스스루, `eval.py:769-793`).
측정 대상 코드 = `master@758574d` 고정, 4개 설정 전부 동일 코드.

| 설정 | 플래그 | 성격 |
|---|---|---|
| A (기준선) | 기본값(CRT 3.0, logprob −2.0) | 같은 세션 기준선 |
| B | `--compression-ratio-threshold 2.5` | 반복 텍스트 억제 강화 |
| C | `--logprob-threshold -1.5` | 저신뢰 억제 강화(조임) |
| D | `--logprob-threshold -2.5` | 억제 완화(풂) |
...
.venv\Scripts\python.exe scripts/eval.py --model-dir whisperlivekit/model/whisper-large-v3-turbo `
  ...
  --diarization --sortformer-model whisperlivekit/model/sortformer-4spk-v2.nemo `
  --compression-ratio-threshold 3.0 --repeat 3 --transcript-dir .omc/transcripts_hallu_A `
  --output .omc/benchmarks/eval_hallu_A_base_N3.json      # B/C/D는 위 표의 플래그로 치환
```
설정당 `--repeat 3`, 순차 실행(VBCable 단일 장치). held-out은 D에 한해 `ytn1+eng1` 단회.

### 정량 결과

**① 노이즈 바닥 — 동일 동작 N=9 (핵심 산출물)**

B(CRT 2.5)는 **9회차 전부 CR 게이트 발동 0회**로, 서버 동작이 기준선과 **완전히 동일한 노이즈 복제 런**임이
로그로 확정됐다. 따라서 A(N=3) + 아침 master 런(`eval_20260818_0807_codeswitch.json`, 동일 코드 `8af6205`,
N=3) + B(N=3) = **동일 동작 N=9**를 하나의 분포로 묶을 수 있다.

| 파일 | WER median | min~max (폭) | stdev | 화자F1 median | 화자F1 범위 | 삽입 median (범위) |
|---|---|---|---|---|---|---|
| bong1 | 29.8% | 19.9~34.6% (**14.8pp**) | 4.3 | 62.5% | **61.1~66.7%** | 29 (18~36) |
| ytn2 | 13.3% | 10.8~21.7% (**10.8pp**) | 3.4 | 88.9% | 63.2~94.7% | 6 (4~16) |
| sbs1 | 8.9% | 8.3~10.7% (2.4pp) | 0.8 | 100.0% | **40.0~100.0%** | 0 (0~2) |

**② 설정별 결과 (N=3, 파일별 median)**

| 파일 | 지표 | A(−2.0) | B(CRT2.5·무발동) | C(−1.5) | D(−2.5) |
|---|---|---|---|---|---|
| bong1 | WER med / max | 30.4 / 34.6 | 27.4 / 29.8 | 22.9 / 27.1 | 23.5 / 34.9 |
| bong1 | **화자F1** med / worst | 61.5 / 61.1 | 61.5 / 61.1 | 66.7 / 62.9 | **78.8 / 64.5** |
| bong1 | 삽입 / 삭제 med | 27 / 42 | 33 / 12 | 23 / 14 | 27 / 10 |
| bong1 | LMR med | 12.0 | 8.7 | 7.6 | 2.2 |
| ytn2 | WER med / max | 18.2 / 21.7 | 13.3 / 14.8 | 14.3 / 14.3 | **12.3 / 12.3** |
| ytn2 | 화자F1 med / worst | 63.2 / 63.2 | 88.9 / 88.9 | 90.0 / 88.9 | 94.7 / 85.7 |
| sbs1 | WER med / max | 8.9 / 8.9 | 10.1 / 10.1 | 8.9 / 9.5 | 10.7 / 10.7 |
| sbs1 | 화자F1 med / worst | 80.0 / 80.0 | 80.0 / 40.0 | **100.0 / 100.0** | 100.0 / 40.0 |
| 3파일 | 오류 총합(삽입/삭제/치환) | 39/57/71 = 167 | 43/23/68 = 134 | 31/22/66 = 119 | 37/19/64 = 120 |

**③ QualityGate 발동 계측 (서버 로그 전수, 회차별)**

| 설정 | bong1 logprob억제 / streak | ytn2 logprob억제 / streak | CR억제 |
|---|---|---|---|
| A (−2.0) | 44·47·57 / 3·3·6 | 44·45·54 / 4·6·5 | **0** |
| B (CRT 2.5) | 55·36·47 / 3·2·1 | 49·29·38 / 2·0·3 | **0** |
| C (−1.5) | 55·57·59 / 4·3·4 | 53·50·43 / 3·3·2 | 0 |
| D (−2.5) | 18·26·44 / **0·0·3** | 26·33·36 / **0·0·0** | 0 |

**④ held-out (D만, 단회)**: ytn1 WER **27.0%** / 화자F1 94.1% / LMR **19.7%** — 기준선 ytn1(N=3, 아침 런)
median 16.0%·범위 8.6~19.0%·LMR 2.6% 대비 **밴드 밖 회귀**. eng1 WER 4.8%(회귀 없음).

### 분석 (전사 내용 정성 대조)

**노이즈 바닥이 미세 판정을 무력화한다(가장 중요)**: B는 서버 동작이 기준선과 **한 글자도 다르지 않은데도**
bong1 median을 30.4→27.4%(−3.0pp), ytn2를 18.2→13.3%(−4.9pp) "개선"시켰고, sbs1 화자F1 worst를
80→40%로 "악화"시켰다. **N=3 median 차이 3~5pp와 화자F1 worst 40~100%p 스윙은 개입 없이도 발생한다** —
Exp-211의 짝지음 N=1 기각 판단과 Exp-210의 ytn2 N=6 재확인이 옳았음이 독립적으로 재확인됐다.

**bong1**(D, R1 기준):
- **환각(은닉 번역) 소멸**: 웃음 구간에서 기준선 A는 `주인공은 참. 하하하하하. 아틀레입니까? Just on them,
  yeah. Yeah. I'm sorry. I'm sorry, yeah. So.` — 실제 발화 `죄송합니다, 형님`이 유실되고 그 자리를 **영어
  필러 은닉번역**이 채웠다. D는 `하하하하. 아틀렘이가. 하하하. 죄송합니다, 형님. 하하하하.`로 원 발화를
  살렸다. A의 `보통은 스쿼트 당신처럼` 2회 중복도 D에서 소멸.
- **화자F1이 노이즈 밴드를 벗어남**: 기준선 9회가 61.1~66.7%(폭 5.6pp)에 촘촘한데 D는 84.8·78.8·64.5.
  그리고 그 3회차의 streak 발동수가 **0·0·3**으로, F1이 뛴 두 회차가 정확히 **폐기 refresh 0건 회차**다
  (설정 내부 1:1 대응 — Exp-210의 "폐기↔피해 단조상관"이 파라미터 축에서 재현).

**ytn2**(D): 3회 전부 streak 0 → WER 12.3·10.3·12.3으로 **분산이 10.8pp → 2.0pp로 붕괴**. worst-case 축소가
§3.8 최우선 원칙에 정확히 부합하는 유일한 관측.

**ytn1(held-out, D)** — **회귀 지점 특정**: 세션 **서두**다. 정답 `안녕하십니까. 지난 세 달 만에 두 번째로
대한민국에 다시 방문할 수 있게 되어 기쁘게 생각합니다.`가 전사에서 `Yeah, I'm not sure It's been a long time
ago, two years ago, to go back to the United States, so I'm happy to think about it.`로 **통째 영어 은닉
번역**됐다(LMR 19.7%의 실체). Exp-178 ① 콜드스타트 실패모드 그대로 — 게이트를 풀자 이 쓰레기가 억제되지
않고 커밋됐다.

**Case B(단어 중간 분절)**: A/C/D 전 전사 430줄 스캔 결과 **0건**(hard-fail 없음).

**이번 변경 영향 요약**: `compression_ratio` 게이트는 이 스트리밍 구조에서 **구조적으로 사문(死文)**이다
(세그먼트가 짧아 압축비가 문턱에 못 미치고, `_quality_gate`가 logprob를 먼저 검사해 걸리면 즉시 반환하므로
CR 검사에 도달조차 못 한다 — `align_att_base.py:1193-1214`). 실제로 동작하는 유일한 억제 레버는 logprob이며,
그 임계값 하나가 **①쓰레기 출력 차단과 ②3연속 시 오디오 폐기 트리거를 겸직**한다. 조이면 ②의 폐기 피해가
늘고, 풀면 ①이 뚫려 콜드스타트 은닉번역이 샌다 — **이 겸직이 이번 스윕이 규명한 핵심 구조**다.

### 채택 조건 판정

- **B (CRT 2.5)**: ❌ **기각 — 무발동**. 9/9 회차 CR 발동 0. 관측된 "개선"은 전부 노이즈(동일 동작 확정).
- **C (logprob −1.5)**: 📋 **보류**. 억제 55~59회로 A(44~57) 대비 증가폭이 작은 **약한 개입**. sbs1 화자F1
  100/100/100(worst 100)·bong1 삽입 23/23/23(밴드 median 29 아래, 이상하리만치 안정)은 신호일 수 있으나
  bong1 화자F1(72.7·66.7·62.9)은 3회 중 1회만 밴드를 넘어 귀속 불가. **held-out 미측정**.
- **D (logprob −2.5)**: ❌ **단독 채택 불가**(게이트 탈락). 게이트 순서대로:
  1. 화자F1 worst 미회귀 → **통과**(bong1 61.1→64.5, ytn2 63.2→85.7; sbs1 40.0은 B 무발동 런과 동일값이라
     노이즈).
  2. WER max 미회귀 → **held-out ytn1에서 탈락**(16.0→27.0%, 밴드 8.6~19.0 밖). 테스트셋만 보면 통과
     (bong1 34.6→34.9 동급, ytn2 21.7→12.3 대폭 개선, sbs1 8.9→10.7 밴드 상단).
  3. LMR(진단) → bong1 12.0→2.2로 개선이나 **held-out ytn1 2.6→19.7로 역전** — 개선이 파일 특화적임을 시사.
- **사용자 목표(환각 감소) 대비**: 삽입(직접 환각 지표)은 어느 설정에서도 밴드를 벗어나지 않았다
  (bong1 밴드 18~36 / A 27 · C 23 · D 27). D가 줄인 것은 **삭제(유실) 57→19**와 **은닉번역(치환형 환각)**
  이며, 순수 삽입형 환각은 이번 스윕의 어떤 파라미터로도 줄지 않았다.

### 결론
**기각 1건(B) · 보류 2건(C·D). master 코드·기본값 전부 무변경**(이번 세션 코드 편집 0건).

CLI 노출 파라미터 축에서 환각을 줄이는 단독 설정은 찾지 못했다. 다만 그 과정에서 두 가지 재사용 가능한
사실을 확보했다 — ① **노이즈 바닥 N=9 실측**(향후 모든 채택 판정의 기준선), ② **QualityGate 임계값의
이중 역할 규명**(억제 강도와 폐기 트리거가 한 값에 묶여 있어 파라미터 하나로는 양쪽을 동시에 만족시킬 수
없다). D의 held-out 회귀는 이 겸직 구조의 직접 증거다.

### 다음 가설
1. **[1순위] 겸직 분리 — `quality_gate_reset_after` CLI 승격 후 스윕**: 억제 강도는 −2.0으로 두고(콜드스타트
   쓰레기 차단 유지) 폐기 트리거만 완화(3 → 4·5·비활성)하면 D의 이익(폐기 소멸 → 화자F1·유실 개선)과
   A의 방어(은닉번역 차단)를 동시에 얻을 수 있다. `config.py:26`에 필드는 있으나 **CLI 미노출**이라 코드
   변경(워크트리)이 필요하다. 이번 스윕이 도출한 가장 구체적인 후속.
2. **콜드스타트 전용 방어**: D의 유일한 실패가 세션 서두(Exp-178 ①)에 집중되므로, 세션 초입 N초에 한해
   게이트를 조이는 조건부 임계값이면 D를 살릴 수 있다. 단 "구간별 하드코딩"이라 §3.8 일반화 원칙과의
   정합성 검토 필요.
3. **C(−1.5) held-out 측정**: 보류 상태 해소용. bong1 삽입 23/23/23의 이상 안정성이 실재하는지 N=3 추가.
4. **CR 게이트 사문화 처리**: 현 구조에서 `compression_ratio_threshold`는 어떤 값을 줘도 발동하지 않는다
   (0-firing 실증). 문서·기본값에서 "반복 세그먼트 억제" 효과를 기대하지 않도록 명시하거나, 검사 순서를
   바꿔(CR 먼저) 살릴지 결정 필요.

### 기록
- Epoch 변화 없음(파라미터 값 실험 + 코드 무변경 — 세대 경계 규칙상 구조 변경 아님).
- 벤치마크: `.omc/benchmarks/eval_hallu_A_base_N3.json` · `eval_hallu_B_crt25_N3.json` ·
  `eval_hallu_C_lp15_N3.json` · `eval_hallu_D_lp25_N3.json` · `eval_hallu_D_heldout.json`.
...
## Exp-217 — Exp-212 C(logprob −1.5) held-out 재검증 — 보류 해소, 안전하나 채택 근거 부족 (2026-08-19) [E8, 기각(비채택)]

**측정 언어모드**: `auto`

### 발단
Exp-212에서 C(`logprob_threshold=-1.5`, 억제 강화 방향)는 held-out 미측정으로 "보류" 상태였다. `reset_after=5`
+ 콜드스타트 fix 조합으로 held-out까지 확정.

### 결과

| 파일 | WER median/max(N=3) | 화자F1 median/worst | 비고(E8 baseline `logprob=-2.0` 대비) |
|---|---|---|---|
| bong1 | 24.1/25.0 | 0.688/0.684 | 기준(22.9/22.9, F1 0.647/0.629)보다 WER 소폭 높으나 F1 worst는 소폭 개선 — 밴드 내 |
| ytn2 | 13.3/14.8 | 0.9/0.762 | 기준(12.8/13.3)보다 소폭 높음 — 밴드 내 |
| sbs1 | 8.9/10.1 | 1.0/0.8 | 기준(10.7/11.9)보다 **개선** |
| ytn1(held-out) | 13.5% | F1 0.762 | **회귀 없음**(LMR 1.3%, 기준 0.0%와 사실상 동일) — Exp-212 D와 달리 **안전** |

### 결론
C는 D와 달리 held-out에서 **catastrophic 회귀가 없다** — "보류" 상태 해소, 안전 확인. 다만 테스트셋 지표가
현 기본값(`logprob=-2.0`)과 견줘 **뚜렷한 우위를 보이지 않음**(파일별로 소폭 개선·소폭 악화가 섞여 있고 전부
기존 노이즈 밴드 내) — sbs1만 개선, bong1·ytn2는 동급 이하. **채택 근거 불충분으로 기본값 유지**(logprob=-2.0
그대로). Exp-212가 남긴 "이상하리만치 안정적인 bong1 삽입 23/23/23" 신호는 이번 N=3(구성이 달라 직접비교
불가, reset_after=5 결합)에서 뚜렷하게 재현되지 않음.
...
## Exp-219 — `quality_gate_reset_after` 6·8 재확인 — 5 대비 우위 없음, 기본값 5 유지 (2026-08-19) [E8, 기각(비채택)]

**측정 언어모드**: `auto`

### 발단
Exp-213 스크리닝에서 6·8이 refresh 0회 발동(가장 안정적으로 보임)을 보였으나 held-out 미검증 상태였다.
이번 세션의 콜드스타트 fix(Exp-214/215) 이후 재확인.

### 결과 + 발동 감사

| 설정 | bong1 med/max | ytn2 med/max | sbs1 med/max | held-out ytn1 | refresh 발동(9회차 합) |
|---|---|---|---|---|---|
| 5(현 기본값, Exp-218 측정) | 22.3/31.0 | 14.3/19.7 | 9.5/10.1 | 11.0 | (미집계, 참고용) |
| 6 | 28.9/32.2 | 14.8/24.6 | 8.9/9.5 | 10.4 | **2/9** |
| 8 | 26.8/31.9 | 12.8/25.1 | 8.9/8.9 | 11.0 | **0/9** |

### 분석
**val=8은 9회차 전부 refresh 미발동**(`[QualityGate] N consecutive suppressions` 로그 0건) — 즉 QG 메커니즘이
사실상 개입하지 않는 상태에서도 raw WER이 5 대비 bong1 median +4.5pp·ytn2 max +5.4pp 나빴다. **이는 순수
노이즈**(Exp-212/208와 동일 방법론 — 0-firing인데 수치가 다르면 개입 무관 변동)이지, reset_after=8 자체가
5보다 나쁘다는 인과 증거가 아니다. val=6도 발동 2/9로 거의 무개입에 가까웠다.

### 결론
6·8 모두 **비채택 — 기본값 5 유지**. 근거: ① held-out은 셋 다 무회귀로 동급(10.4~11.0%) — 차별화 신호
없음. ② 테스트셋 raw 수치는 5가 앞서 보이나 6·8이 거의 0-firing이라 그 차이를 reset_after 값의 인과
효과로 귀속할 근거가 없다(발동 자체가 희소해 "더 높였을 때 어떻게 달라지는지" 자체가 관측 불가). ③ 5는
이미 Exp-214/215에서 N=3 확정+held-out 통과로 채택된 값이라, 추가 상향이 신호 없이 리스크만 더할 이유가
없음. **이 파라미터 축은 5에서 종결** — 추가 재시도 없음.

### 기록
- Epoch 변화 없음. 벤치마크: `eval_resethi{6,8}_confirm_*.json` · `eval_resethi{6,8}_heldout_*.json`.
- 전사: `.omc/transcripts_resethi/{v6,v8}_{confirm,heldout}/`. 측정 시각 2026-08-19 14:49~15:37대.

---

## Exp-220 — 기존 AnchorRepeatFilter 열거/anaphora 오탐 위험 감사 (2026-08-19) [E8, 계측완료·비대상·조치 없음]

**측정 언어모드**: 코드 직접 호출 검증(경로 C 미사용, N/A)

### 발단
Exp-218 "다음 가설 3"에서 미확인으로 남긴 사항 — 기존 `_find_anchor_repeat_storm`(Exp-169)이 순수 열거·긴
스템 anaphora에도 `True`를 반환할 수 있다는 점을 이번 세션 신규 게이트 개발 중 부수 관찰했다. 실사용
빈도(§3.8 대상 도메인 = 격식체 한국어 브리핑, "첫째·둘째·셋째"류 열거가 흔함)를 고려해 실위험 여부를 확인.

### 방법
`_find_anchor_repeat_storm`을 코드에서 직접 호출해 두 계열 입력으로 대조:
1. **완전 동일 꼬리 열거**("첫째 ~ 협의했습니다 둘째 ~ 협의했습니다 셋째 ~ 협의했습니다 넷째 ~ 협의했습니다",
   4항목 전부 동일 어미) — 실제 열거지만 어미가 부자연스럽게 통일된 극단 케이스.
2. **자연스러운 열거**(각 항목 어미가 실제 발화처럼 다름 — "강화하기로 했습니다"/"확대하기로 결정했습니다"/
   "추진하기로 합의했습니다"/"정례화하기로 했습니다" 등, 3~4항목).

### 결과
- ①은 **`True`(storm으로 오판, 실제 발화라면 드롭됨)** — 긴 스템 anaphora 합성 예시도 동일하게 `True`.
- ②(자연스러운 어미 변주)는 **전부 `False`**(정탐) — 정상 통과.

### 결론
오탐 위험은 **실재하나 좁다** — 열거 항목의 결말(꼬리 n-gram)이 **여러 항목에 걸쳐 우연히 완전 동일**할
때만(자연 발화에서 흔치 않음) 발동한다. 이번 세션 테스트셋(bong1/ytn2/sbs1/kor1~3/ytn1/eng1) 전사에서
이 패턴에 의한 삭제 흔적은 관측되지 않았다(§3.8 하드코딩 금지 원칙상 특정 문구로 우회하는 수정은 부적절 —
Exp-218의 LCP 방식과 유사한 재설계가 필요하나, 기존에 검증된 필러 storm 포착력을 깨뜨릴 위험이 있어 이번
세션 범위에서는 **수정하지 않는다**). **조치 없음 — 감시 대상으로만 기록.**

### 다음 가설
실측 데이터(경로 C 전사)에서 열거형 삭제(정답에는 있는 "첫째/둘째/셋째" 문단이 전사에서 통째로 빠지는
패턴)가 관측되면 이 게이트를 1순위 용의선상에 올릴 것 — `[AnchorRepeatFilter]` 로그와 해당 구간을 대조.
...
## Exp-222 — 스윕 S1 v2 완주(잔여 4 arm) + `--vad-threshold` 상향 스윕 4 arm 신규 (2026-08-20) [E8, vad 축 종결·기본값 무변경]

**측정 언어모드**: `auto` 단일 (스크리닝 6파일 전부 `--lan auto`)

### 발단
Exp-221이 시간예산 소진으로 23/27 arm에서 멈췄다. ① 잔여 4 arm(D3-min-chunk-size 0.2/0.3 ·
D4-rewind-threshold 100/400)을 완주하고, ② **사용자 요청으로 `--vad-threshold` 상향 스윕을 신규
추가**했다 — 기존 A3는 기본값 0.3의 **아래**(0.2·0.25)만 측정돼 축이 반쪽이었고, 0.25가 유력 후보로
남아 있어 "위 방향은 왜 안 보는가"가 미해결이었다. ③ 이어서 S2(`repeat=16`) 확정 측정에 착수했으나
사용자 지시로 중단했다.

### 방법
- 경로 C(VBCable), `--lan auto`, diar-ON(Sortformer), CRT 3.0, **`repeat=2`**, 스크리닝 6파일
  (bong1/ytn2/sbs1/kor2/kor3/eng1). **코드 변경 0줄** — CLI 파라미터만 바꾼 OFAT 스윕.
- 상향 4점은 하향과 같은 0.05 해상도(0.35/0.4/0.45)에 0.5를 추가해 추세 확인. arm 목록 신규 파일
  `docs/research/2026-08-20_param-sweep-arms-vad-up.json`, 원장은 Exp-221과 동일 원장에 이어 기록.
- 실행: 잔여 4 arm 11:03~12:53(1h 50m) → vad 상향 4 arm 12:53~14:43(1h 50m). 하니스 오류 0건
  (Exp-221에서 고친 dist staleness·스테일 `--server-frontend-dir`가 재발하지 않음을 확인).

### 테스트 세트 결과

파국 = 파일별 E8 baseline median +8%p 초과 회차 수 / 총 회차. WER은 2회 평균(%).

| arm | 파국 | bong1 | ytn2 | sbs1 | kor2 | kor3 | eng1 |
|---|---|---|---|---|---|---|---|
| **BASE-0**(Exp-221 대조군) | 1/12 | 28.8 | 14.0 | 8.0 | 19.7 | 37.7 | 4.8 |
| D3-min-chunk-size-0.2 | 1/12 | 30.7 | 17.0 | 8.9 | 17.6 | 34.8 | 4.3 |
| D3-min-chunk-size-0.3 | 1/12 | 24.7 | 11.8 | **14.0** | 14.5 | 31.5 | 5.2 |
| **D4-rewind-threshold-100** | **0/12** | 25.0 | 11.8 | 9.5 | 16.2 | 35.8 | 4.3 |
| D4-rewind-threshold-400 | 1/12 | 22.0 | **19.0** | 9.5 | 16.9 | 34.1 | 5.7 |
| A3-vad-threshold-0.35 | 2/12 | 31.6 | 10.8 | 8.3 | 15.2 | 31.1 | 5.2 |
| A3-vad-threshold-0.4 | 1/12 | 27.0 | 12.6 | 8.6 | 16.2 | 34.4 | 5.2 |
| A3-vad-threshold-0.45 | **0/12** | 26.7 | 14.3 | 9.5 | 18.6 | 31.5 | 5.7 |
| A3-vad-threshold-0.5 | **0/12** | 25.3 | 12.3 | **11.6** | 16.2 | 34.4 | 5.2 |

**`--vad-threshold` 축 전체**(Exp-221의 하향 3점 + 이번 상향 4점 = 7점, `anchor`=`anchor_repeat`):

| 값 | 0.2 | 0.25 | **0.3(기본)** | 0.35 | 0.4 | 0.45 | 0.5 |
|---|---|---|---|---|---|---|---|
| 파국 | 2/12 | 1/12 | 1/12 | 2/12 | 1/12 | 0/12 | 0/12 |
| bong1 | 33.0 | 24.7 | 28.8 | 31.6 | 27.0 | 26.7 | 25.3 |
| ytn2 | 10.1 | **9.4** | 14.0 | 10.8 | 12.6 | 14.3 | 12.3 |
| sbs1 | 8.6 | 10.4 | **8.0** | 8.3 | 8.6 | 9.5 | **11.6** |
| kor3 | 39.4 | 35.1 | 37.7 | 31.1 | 34.4 | 31.5 | 34.4 |
| anchor | 37 | 17 | 35 | 57 | 38 | 8 | 6 |

무출력(전사 0자)은 8 arm 전부 0건. 거동 카운터가 BASE-0와 동일한 noise-control arm도 0개.

### 분석 (전사 내용 정성 대조)

> 대조 기준: arm·파일별 **2회 중 WER이 높은 회차**(보수적 선택).

**sbs1** — vad 상향의 악화 기전이 텍스트로 확인된다:
- **단어 오치환+유실**: VAD-0.5 전사 `"프론슨의 지상군은 항상 그곳에 남는다며"` / 정답 `"그러면서
  지상군은 항상 그곳에 남는다며"` — 발화 시작을 놓쳐 `"그러면서"`가 앞 문장 고유명사로 대체됐다.
  전사 길이도 723자로 BASE-0(749)·정답(754) 대비 짧다. **VAD를 둔감하게 하면 여린 발화 시작을
  놓친다**는 기전과 정합.
- **자막 크레딧 환각**(D3-0.3, sbs1 18.5% 회차의 원인): 전사에 `"한글자막 : 박진희/영상편집 박진희,
  박진희 기자가 보도합니다."`가 삽입 — 정답에 없는 Whisper 자막코퍼스 환각. D3 열위의 실체다.

**kor3** — **반복 4-gram 지표의 오탐 사례**:
- 거의 모든 arm에서 `"현 구조를 유지한 상태에서"`가 반복 4-gram으로 잡혔으나, **정답에 2회 나오는
  실제 발화**였고 전사는 1회뿐이었다(= 반복이 아니라 **유실**). 자동 반복 탐지 수치를 환각 근거로
  쓰기 전에 반드시 정답을 대조해야 한다(CLAUDE.md §4 "원본 발화 확인 규칙"의 자동지표판).

**bong1** — **Exp-221 정성 결론의 정정(중요)**:
- Exp-221은 A4-vac-chunk-size-0.5의 근거로 "코드스위칭 경계 유실 회복"(`"누가"`·`"많이 했어요"`
  복원)을 들었다. 이번에 **31 arm × 2회 = 62 회차 전수 검사**한 결과 그 구간 유실은 **BASE-0 R2 ·
  D2-audio-max-len-20 R2 · A3-vad-0.45 R1 단 3건(4.8%)**뿐이고 나머지 59회차는 arm 종류와 무관하게
  전부 정상 전사였다. `"who is the main"`은 **62/62 전부 정상**.
- → **그 "회복"은 arm 효과가 아니라, 하필 BASE-0의 worst 회차(WER 36.7%)와 대조한 아티팩트다.**
  A4-0.5에 남는 근거는 정량 신호(파국 0/12·WER)뿐이며, 정성 근거는 철회한다. **1회 대조로 정성
  결론을 내리면 안 된다**는 방법론 교훈(§ 이번 변경 영향).

**이번 변경 영향**: 코드 변경이 없으므로 위 실패 모드는 각 knob의 효과이거나 회차 노이즈다.
vad 상향은 sbs1에서 "발화 시작 놓침"을 실제로 만들었고(기전 정합), 나머지 파일에서는 지표 변화가
노이즈 밴드를 넘지 못했다. Case B(단어 중간 분절)는 대조한 6 arm 전 파일에서 0건.

### 채택 (조건) 판정
- **①최악 케이스 미회귀 / ②median 개선 판정은 이번 단계에서도 적용 불가** — S1은 스크리닝
  (`repeat=2`)이라 max/median을 분리할 표본이 없다. **어떤 arm도 채택하지 않았고 기본값은 하나도
  바꾸지 않았다.** 채택 게이트는 S2(`repeat=16`)에서만 적용한다.

### 결론
1. **`--vad-threshold`는 기본값 0.3 유지 — 축 종결.** 0.2~0.5 7점에서 파국률(2·1·1·2·1·0·0)은
   BASE(1/12) ±1의 노이즈 범위이고, WER도 Exp-212 무개입 밴드(bong1 19.9~34.6 · ytn2 10.8~21.7 ·
   sbs1 8.3~10.7) 안이며 단조 추세가 없다. 파일별 최적점이 제각각(ytn2=0.25, sbs1=0.3, kor3=0.35)인
   것 자체가 노이즈의 특징이다. 유일한 방향성은 **sbs1 상향 악화**(8.0→8.3→8.6→9.5→11.6, 0.5는
   밴드 상단 초과)뿐이라 상향할 이유가 없고, 하향 0.25도 ytn2 9.4(전 arm 최저) 대신 kor2 24.5
   (최악)를 치르는 트레이드오프라 S2 슬롯 가치가 없다. **Exp-221의 "0.25 차순위 후보" 지위를 철회**.
2. **`--min-chunk-size`(D3)는 건드릴 이유 없음** — 0.2·0.3 모두 BASE 동률(1/12)이고 개선 신호 없음.
   실험 0건이던 knob이 이로써 정리됐다.
3. **`--rewind-threshold 100`을 S2 승격 후보에 추가** — 파국 0/12, 역방향 파일이 sbs1(+1.5%p)
   하나뿐이고 bong1 −3.8 · ytn2 −2.2 · kor2 −3.5 · kor3 −1.9%p. anchor_repeat 35→4. (400은 열위.)
4. **거동 카운터는 성능의 대리 지표가 아니다** — vad 0.35→0.45에서 anchor_repeat이 57→8로 급감했는데
   WER·파국률은 따라오지 않았다. 거동 카운터는 "이 arm이 noise-control인가"의 판정에만 쓴다.

### S2 착수·중단 (진행분 유실)
14:43 `A4-vac-chunk-size-0.5` S2(`repeat=16`)에 착수해 bong1 16 + ytn2 16 + sbs1 11 = **43/96 run**
진행 후 16:37 사용자 지시로 중단했다. **`eval.py`가 결과 JSON을 전 회차 종료 후 한 번에 쓰기 때문에
이 진행분은 전량 유실**됐다(원장 flush도 arm 단위, 서버 로그만 잔존하나 WER은 UI 스크래핑 산물이라
복원 불가). 실측 페이스는 run당 2.65분 = **arm당 약 4.2시간**. → **백로그: `eval.py` 회차 단위 결과
flush**(S2를 12시간 돌릴 계획이면 선행하는 편이 기대값이 높다).

### 다음 가설
1. **S2 재개**(`repeat=16`, 약 12~13시간) — 1순위 A4-vac-chunk-size-0.5, 2순위
   A5-min-real-silence-secs-0.6, 3순위 D4-rewind-threshold-100. 재개 명령·사전점검은 진행 리포트
   § 재개 절차(2026-08-20 중단). **여기서 처음으로 채택 게이트를 적용**한다.
2. A4·A5는 "과잉 방출 억제"라는 같은 기전을 공유 → S2 통과 시 2-factor 조합도 후보.
3. 계측 결함 2건(`refresh_segment` 사문화 · `qg_preserve`=`qg_reset` 중복) 수정 — 단 결론 4번대로
   거동 카운터는 성능 지표가 아니므로 S2 착수의 선행조건은 아니다.

### 기록
- Epoch 변화 없음(**E8 유지**) — 코드 변경 0줄, 기본값 무변경.
- 하니스 세대 **H2**(배포 React UI 스크래핑). 측정 언어모드 `auto`.
- 원장(SoT): `docs/research/2026-08-19_param-sweep-ledger-v2.json` — S1 **31 arm 전부 `done`**.
- arm 목록(신규): `docs/research/2026-08-20_param-sweep-arms-vad-up.json`
- 진행 리포트(재개 절차 포함): `docs/research/2026-08-19_param-sweep-progress-v2.md`
- eval JSON 8개: `worktrees/param-tuning-campaign/.omc/benchmarks/eval_20260820_*_S1.json`(로컬)
