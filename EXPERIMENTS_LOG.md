...
### 다음 가설
- Exp-199/200 이전 커밋(구체적으로 `align_att_base.py`의 커밋 절단 로직 변경 전)으로 kor3를 재측정해 "감사합니다"가 실제로 방출됐었는지 대조하면 인과관계를 확정할 수 있다(시간 남으면 시도, 아니면 다음 세션 과제).
- 계열②를 완전히 닫으려면 더 많은 반복(N≥10)과 다른 ko 세션(kor1/kor2)·auto 파일에서도 "필러가 후보로만 생기고 커밋은 안 되는지" 동일 패턴을 확인해 일반화 여부 판단.
