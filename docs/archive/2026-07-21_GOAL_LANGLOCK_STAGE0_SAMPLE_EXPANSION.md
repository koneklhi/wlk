> # ⛔ 실행 완료 — 이 프롬프트를 다시 붙여넣지 말 것 (2026-07-21 아카이브)
>
> **결과: §3 “위험” 분기 선택 → 안 A 완료, 안 B 미착수.** Exp-195로 기록됨.
>
> - **안 A(표본 확대)는 사전 등록 범위를 초과 달성**했다. §2-1이 요구한 스크리닝 6파일에 더해
>   ko(kor1~3)·en(eng1) 양쪽을 `--repeat 3`까지 확증했다 — **총 16세션**.
> - **§3 안전 조건은 불충족**: ko 단일언어 세션 12개(3756 프로브)에서 `p_opp≥0.97` 발동
>   **23건(0.61%)**, 그중 **순수 오탐 2건 확정**(kor2 t=3.99s p_opp=0.9921 `먼저/육군`,
>   kor3 t=12.95s p_opp=0.9969 `원/거리/타격` — 둘 다 정상 전사 구간). 동일 지점이 3/3 회차
>   반복되는 **결정론적** 패턴이라 표본 우연이 아니다.
> - 따라서 §3 처방대로 **§4·§5(안 B Stage 1 섀도우)에 착수하지 않았고, 게이트 파라미터도
>   조정하지 않았다.** master 머지 없음. 코드 변경 0(`whisperlivekit/`·`scripts/`·`tests/` diff 없음).
> - **완화 사실**: 발동 23건 전부 `seglen ≤ 1.02s`·최대 연속 K=2라 인계서 §4 게이트
>   (`seglen≥2.0s ∧ K≥3 ∧ T≥1.0s`)에 **23/23 전부 걸린다**(오프라인 재현). 반대로 ytn1
>   참양성 run 2개는 통과한다. 즉 남은 판단은 정보 부족이 아니라 **설계 수용 여부**다.
>
> **다음 세션이 읽을 것** (이 파일 말고):
> `docs/backlog/LANG_LOCK_STAGE0_HANDOFF.md` §4-0 / §4-0-1 / §4-0-2 (인계·결정 대기 항목) ·
> `docs/research/SOT_LANG_PROBE_STAGE0.md` §10 (분석 정본) · `EXPERIMENTS_LOG.md` Exp-195.
>
> 재개 시 권고 ①(N≥3 확증)은 **이미 완료**됐으므로 곧바로 ② 안 B(섀도우 `would_fire`)로 갈 수 있다.
> 단 그 착수 자체가 사용자 판단 사항이다.
>
> ---
> *아래는 실행 당시의 원본 프롬프트다 — 역사 기록으로만 보존한다.*
