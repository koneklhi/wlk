...
> **⚠️ 갱신 이력(코드 재대조 결과 — 2026-07-13)**: 2026-07-12 브랜치(`feat/frontend-transcript-history`) 머지로
> 아래 기능이 추가·변경됐다(§2.2·§2.3·§7·§10 본문에 이미 반영):
> 1. **세션 전체 히스토리 유지**: 이후 2026-07-13 변경으로 **서버 리텐션이 5분 슬라이딩 윈도우 → 무제한으로
>    바뀌어**, 서버가 세션 전체 확정 히스토리를 그대로 재전송한다 — React 등에서 전체교체 렌더만 해도 히스토리가
>    유지되며 **프론트 누적은 더 이상 필수가 아니다**(§2.2 경고 문단 참고). 내장 UI가 `finalized` 줄을
>    `start|end|speaker` 복합키로 누적하는 코드는 이제 무해한 참조 구현일 뿐이다.
