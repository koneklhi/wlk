...
- **한계**: 일시중단(⏸) 후 재개하면 WebSocket 세션이 새로 시작되고 이전 구간은 클라이언트에만 남으므로
  ...
  - 이 두 사각지대(창 밖 오래된 문장·일시중단 이전 구간)는 **§3.6 `POST /api/retranslate`로 한 문장씩
    개별 재번역**할 수 있다. 세션과 무관한 1회성 호출이라 서버가 이미 잊은 구간에도 쓸 수 있다.

### 3.6 블록 재번역 — `POST /api/retranslate`

텍스트 1건을 **지금의 용어집으로** 다시 번역한다. 관리자 페이지의 '다시 번역' 버튼이 호출한다.
§3.5의 소급 재번역 창(기본 최근 20줄)이 닿지 않는 문장을 개별로 되살리는 용도다.

**세션과 무관한 1회성 호출**이다 — 진행 중 WebSocket 세션·`TranslationManager` 캐시를 전혀 건드리지
않는다. 따라서 서버는 "어느 블록"인지 모르며, **결과를 화면에 반영하는 책임은 클라이언트에 있다**
(서버는 다음 tick에도 기존 번역을 계속 실어 보낸다).

요청:

```json
{ "text": "우리 공군은 준비되어 있습니다.", "src_lang": "ko" }
```

- `src_lang`은 생략 가능(기본 `"ko"`). 틀려도 서버가 문자 구성 기반으로 보정하므로
  (`translator.resolve_src_lang`) 클라이언트가 정확한 언어를 몰라도 된다.

응답:

| 상태 | 본문 | 의미 |
|---|---|---|
| 200 | `{"status":"success","translation":"..."}` | 정상 |
| 200 | `{"status":"warning","translation":"","message":"..."}` | **에코 폐기** — 원문을 그대로 뱉는 응답이 2회 연속. 화면 번역을 덮어쓰지 말 것(덮어쓰면 번역이 사라진 것처럼 보인다) |
| 400 | `{"detail":"..."}` | `text`가 비었음 |
| 502 | `{"detail":"..."}` | 번역 서버에 연결 불가 |
| 503 | `{"detail":"..."}` | `--no-llm-translation`으로 번역 기능이 꺼져 있음 |
| 504 | `{"detail":"..."}` | 번역 서버 무응답(60초). `TranslatorBase`의 httpx 클라이언트는 `timeout=None`이라 서버가 상한을 건다 |

- **호출측 타임아웃**: LLM 왕복 + 첫 호출의 RAG(bge-m3) 로드까지 감안해 **60초**를 잡아야 한다.
  배포 UI 공용 `fetchJson`의 기본값 5초로는 완주하지 못한다.
- **결과가 이전과 같을 수 있다**: 번역 백엔드는 `temperature: 0`이라 같은 원문 + 같은 용어집이면
  같은 번역이 나온다. 실제로 결과를 바꾸려면 §3.4로 번역용어를 먼저 등록해야 한다. 이 엔드포인트가
  확실히 값을 내는 경우는 ① 번역이 비어 있던 문장의 복구 ② 용어집 변경 후 재적용 ③ 소급 창 밖·
  일시중단 이전 구간이다.
