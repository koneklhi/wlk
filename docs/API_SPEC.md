...
| `mode`     | string | 서버 `--ws-protocol`(기본 `delta`) | 출력 프로토콜(§2.4.2). `delta`=snapshot 1회+이후 diff(**기본**), `full`=매 메시지 전체 스냅샷(구 동작 opt-out). `diff`는 `delta`의 하위호환 별칭. 허용값 외는 경고 로그 후 서버 기본값 폴백 |

> **`mode=full` opt-out**: 델타 누적을 아직 구현하지 않은 클라이언트는 `?mode=full`로 pin해 기존 전량 전송
> 스키마를 그대로 받을 수 있다(마이그레이션 경로). 다만 세션이 길어질수록 페이로드·재렌더 비용이 선형 증가하므로
> 임시 수단이다. 실제 적용된 프로토콜은 `config` 메시지의 `protocol` 필드로 통지된다(§2.4.1).
...
- `type: "snapshot" | "diff"` → **상태 메시지(델타 모드 = 기본)**(§2.4.2)
- `type` 없음 → **상태 스냅샷(full 모드 = `?mode=full` opt-out)**(§2.4.2)

#### 2.4.1 제어 메시지

| `type`          | 시점           | 페이로드                                                  | 의미                                                                            |
| --------------- | -------------- | --------------------------------------------------------- | ------------------------------------------------------------------------------- |
| `config`        | 연결 직후 1회  | `{"type":"config","useAudioWorklet":false,"protocol":"delta","mode":"delta","language":"auto"}` | 오디오 송신 형식 + **적용 프로토콜** + **세션 적용 언어** 통지. `useAudioWorklet`은 배포 기본값에선 고정(구현 불필요) |
...
#### 2.4.2 상태 메시지 — 델타(기본) / full(opt-out)

매 처리 사이클(~50ms) 중 **직전 상태와 내용이 다를 때만** 전송된다. `lines[]`는 **세션 전체 확정 히스토리를
무제한 유지**한다(§6 리텐션) — 다만 **그 전체를 매번 보내지는 않는다.**

##### 델타 프로토콜 (기본, `--ws-protocol delta`) — ⚠️ 계약 변경

> **변경 이력**: 이전에는 매 메시지가 전체 상태 스냅샷이었고 "매 메시지 전체 교체 렌더"가 권장이었다. 세션이
> 길어질수록 WebSocket 페이로드와 전체 재렌더 비용이 무한히 커져 전사가 버벅이므로 **기본 전송을 델타로 전환**했다.
> **클라이언트는 이제 상태를 누적해야 한다.** 누적 구현 전이라면 `?mode=full`로 pin(§2.1).
...
세션 길이에 비례해 커지므로 **마이그레이션용 임시 경로**다.
...
**델타 프로토콜(기본)에서 이 재조정은 `new_lines` 꼬리 재전송으로 나타난다.** §2.4.2의 꼬리 교체를 그대로
수행하면 위 ①②가 자동으로 맞춰진다 — 재조정된 줄이 **같은 `id`로 갱신 내용을 실어 다시 오기 때문**이다.
`?mode=full` opt-out에서는 매 스냅샷 `lines[]`를 통째로 다시 그리면 동일한 결과가 된다.
...
| WS 경로              | `/asr`                      | —                                                                             |
| **WS 출력 프로토콜** | **`delta`(기본)**           | 서버 `--ws-protocol {delta,full}`; 세션별 `?mode=delta\|full` 오버라이드(§2.1·§2.4.2). `snapshot` 1회+이후 `diff` — **클라이언트 누적 필수** |
...
- OpenAI/Deepgram 호환 계층: [0.Metafile/docs/API.md](../0.Metafile/docs/API.md)
  (⚠️ 이 upstream 문서는 diff 프로토콜을 "`?mode=diff` 옵트인"으로 기술한다 — 본 저장소에서는 **delta가 기본**이다.
  델타 계약의 정본은 위 §2.4.2다.)
