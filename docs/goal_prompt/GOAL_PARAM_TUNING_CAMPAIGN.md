...
| 단계 | 대상 | 세트 | arm당 | 총 |
|---|---|---|---|---|
| **S1 거친 스크리닝** | 전 arm ~55 | 저분산 4파일 **N=1** | ~8분 | **~7.5시간** |
| **S2 정밀 스크리닝** | S1 생존 ~15 | 저분산 4파일 **N=3** | ~24분 | ~6시간 |
| **S3 채택 확정** | S2 상위 3~5 | 테스트 6파일 **N=3** + held-out(ytn1·eng1) 단회 | ~45분 | ~4시간 |
| **S4 최종 검증** | 최종 후보 1~2 | bong1 **N=5** + Track F ablation | ~15분/회 | ~3시간 |

**S1 가지치기 (관대하게)**: N=1은 저분산 파일에서도 ±3%p 폭이 있다. **어느 파일이든 +8%p 이상 회귀한
arm만 탈락**시킨다. 미세 차이로 자르지 않는다 — N=1은 규정상 '방향 신호'일 뿐이다.

**S2 통과 기준**: 저분산 4파일 sd 1.4~1.5%p → **2×sd ≈ 3%p**가 잡음 밴드.
- 4파일 중 **어느 것도 median WER +3%p 이상 회귀하지 않을 것**
- 최소 1개 파일에서 median WER **-3%p 이상 개선** 또는 화자F1 개선
- **knob이 실제로 거동을 바꿨다는 로그 증거**가 있을 것

**S3 채택 게이트 (CLAUDE.md §4 순서 엄수)**
...
- 고분산 파일(bong1·kor3)은 median이 아니라 **최악값·파국 발동률**로 읽는다
  ...
  "arms": [
    {
      ...
      "behavior_evidence": { "silence_tokens": 143, "refresh_segment": 12 },
