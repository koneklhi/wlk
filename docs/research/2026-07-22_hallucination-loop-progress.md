...
**최종 결론**: **조건부 채택권고(0.2s 버전)로 재상향**. 단 kor1 flip-flop 억제력이 0.2s에서도 유지되는지는 이번 재측정 표본이 우연히 0-firing이라 미재확인(다음 세션에서 버스트 재현 회차로 확인 필요) — ytn2·sbs1도 0.2s 기준 재확인 필요, 정식 `--repeat 3` 확정측정도 0.2s로 재수행 필요. `/log-experiment` 기록 완료(Exp-204, 전체 정정 이력 포함).

**교훈**: 기존 프로젝트 상수(Exp-188의 `MIN_SPEAKER_ATTRIBUTION_SECS=0.5`)를 "동일 문제니 동일 값이 맞겠지"라고 검증 없이 재사용한 것이 근본 실수였다 — **적용 계층이 다르면 같은 상수라도 재검증이 필요**하다는 구체적 사례로 기록.
