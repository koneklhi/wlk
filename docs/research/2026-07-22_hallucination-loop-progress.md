...
한 문단 요약: 원 goal의 T1/T2/T3 큐를 전부 소진하고(T1 채택권고, T2 게이트기각, T3 신규계열발견), 그 과정에서 발견한 kor1 flip-flop 근본원인을 T5로 별도 수정했다. T5는 1차 시도(0.5s 문턱)가 §3.8 최우선 파일 bong1을 오히려 해친다는 것을 **플레인 마스터 동일조건 대조측정**으로 직접 확인하고, 문턱을 0.2s로 재설계해 해결했다 — 이 과정 자체가 "채택권고→철회→재설계→재상향"이라는 정직한 반복 검증의 기록이다. T1·T5 각각 채택권고 수준이며 결합 시 상호보완도 확인했으나, kor1 flip-flop이 워낙 간헐적이라(이번 세션 kor1 반복측정 20여 회 중 버스트 재현 3회뿐) "결합 브랜치에서의 완전 억제" 최종 확인만 다음 세션 과제로 남았다. **가장 실질적인 산출물은 T1+T5(0.2s) 두 브랜치의 채택권고**이며, 나머지(T2/T3/T4 및 신규 발견 계열들)는 진단·계측 자산으로 다음 개선 사이클의 출발점이 된다.

## 상단 요약 (매 이터레이션 갱신)

- **루프 시작**: 2026-07-22 22:53 · **경과**: ~5시간 15분 (갱신 시각 04:08 기준, 2026-07-23 — 6시간 도달 04:54 예정) · **테스트 4개 실험 브랜치(T1/T2/T3/T5) 최종 재검증 전부 GREEN**(744/753/748/750 passed).
- **전체 상태**: **T1·T2·T3·T4·T5 전부 완료 + T1+T5 결합검증 완료(2회, 0.5s·0.2s 각각) + T2 후속(★계열② 불확실성) + T5 후속(★★★bong1 회귀 원인규명+재설계로 해결)**. 큐 소진 — 종료 시퀀스 진입, 사용자 확인 대기 항목 다수. 종료 직전 kor1 결합 브랜치 9번째 측정도 0-firing(§It-6)으로 마무리 — 버스트 포착은 다음 세션 과제로 확정 이월.
- 3줄 요약:
...
- **추가 5회 재시도**: 결합 브랜치로 kor1을 5회 더 반복(누적 8회 전부 0-firing, WER 12.3~18.1% 무관 밴드). Sortformer의 flip-flop은 VBCable 실시간 재생 타이밍 지터에 좌우되는 것으로 추정되며(T1/T5 둘 다 Sortformer 출력 자체엔 관여하지 않고 그 이후 소비 단계만 손대므로, 코드 분기가 버스트 발생률 자체를 바꿀 이유가 없음), **결합 브랜치가 유독 버스트를 못 잡은 것은 순전히 표본 운으로 판단** — 브랜치별 특성 차이가 아님.
- **루프 종료 직전 9번째 시도(최종)**: 04:04, N=1 1회 추가(NewSpeaker 0회·Refresh 3회·WER 14.6%) — **결합 브랜치 kor1 누적 9회 전부 0-firing**으로 이번 루프 종료. 이번 루프에서는 "결합 시 버스트 완전억제"의 직접 실측 확인을 못 했고, 이는 정직하게 미해결 항목으로 남긴다(메커니즘상 T1 단독 결과(스킵 3/3)와 T5 단독 결과(억제 22→12)를 조합하면 성립해야 하나, 결합 브랜치에서의 동시 실측은 끝내 없었다). JSON: `.omc/benchmarks/eval_final_kor1_t1t5.json`.
