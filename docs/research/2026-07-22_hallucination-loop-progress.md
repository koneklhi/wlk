...
- **루프 시작**: 2026-07-22 22:53 · **경과**: ~2시간 20분 (갱신 시각 01:11 기준, 2026-07-23)
- **전체 상태**: **T1 완료·채택권고**. **T2 완료(계측전용)**. **T3 완료(계측전용, 신규 환각 계열 발견)**. **T5(신규, 큐 T5 예비항목에서 승격) — ChangeSpeaker dispatch 노이즈 필터 구현·커밋, ko 스크리닝에서 flip-flop 대폭 감소 확인, auto 무회귀 측정 진행 중**.
- 3줄 요약:
  1. T1(lang_locked new_speaker 재디코딩 스킵) — TDD 재현·수정 + 실측 확증 완료. **브랜치 커밋(`9c17e0f`) → 채택권고, 머지는 사용자 확인 대기**(Exp-201).
  2. T2(침묵 클로징 환각 섀도우 계측) — would_hold 3건 전수 라벨링 결과 **전부 오탐** — 게이트 배선 금지(Exp-202).
  3. T3(음차 경계 고아 토큰 계측) — **ytn2에서 반대방향 신규 사례 포착**: "정경두"→"John Gyeong-du" 영어 음차(§계열③ 양방향 확장, Exp-203). 자동 임계값은 미도출.
  4. **T5(신규) — kor1 flip-flop 근본원인 규명·수정**: `_update_diarization_state`의 ChangeSpeaker dispatch에 duration 필터가 전혀 없어(기존 완화책 Exp-188은 다른 계층에서만 작동, 실측으로 무력 확인) Sortformer 노이즈가 그대로 스팸됐다. 최소 세그먼트 길이(0.5s) 필터를 TDD로 구현·커밋(`80f4bbd`) — ko 스크리닝 kor1 NewSpeaker 22~30회→4회로 대폭 감소(N=1). auto 무회귀(ytn2 특히 — Exp-106 유사기법 회귀 전력) 측정 중.
  5. 하니스 버그 2건 발견·즉시 수정(모델/데이터 하드링크 누락, T1/T3 각각 1회) — 아래 §하니스 노트.
...
### It-5. T5(신규, §3 예비항목에서 승격) — ChangeSpeaker dispatch 노이즈 필터 [auto N=1 이상치 발견 → repeat=3 재검증 중]
...
- **측정 2 — auto 무회귀(bong1/ytn2/sbs1, `--lan auto`, N=1)**: WER bong1 31.6%(게이트 30.5% 소폭초과 +1.1pp)·**ytn2 21.7%(게이트 34.5% 이내 — Exp-106 유사 회귀 우려는 이번 회차 미재현, 오히려 개선 방향)**·**sbs1 19.6%(게이트 16.1% 초과 +3.5pp, seg_f1도 0.29로 낮음)**.
  - **sbs1 이상 항목 3건 정성 확인**: ① `사태리트의` — sbs1의 **기존·독립적** 계열③ 음차 환각 재현(§T3 진단과 동일 현상, "사태라"의 변주형) — 내 변경과 무관, 사전에 알려진 sbs1 고유 취약점의 자연 재현(5회 중 ~3회 재현률). ② `영상편집:이재명` — 같은 화자(diar `speakers=(1,1)`, 화자전환 무관) 구간에서 발생한 **방송 자막류 환각**(Whisper의 알려진 실패모드, ChangeSpeaker dispatch와 무관함을 로그로 확인). ③ `동맹국들이…방어선이라는 겁니다` 문장 중복(2회 연속 재생성) — 인접 두 infer() 호출이 겹치는 오디오를 재전사한 일반적 스트리밍 중복으로 보이며, 근처에 NewSpeaker/명시적 refresh 로그 없음(내 필터가 유발했다는 직접 증거 없음).
  - **판정 보류**: 3건 모두 내 변경과의 인과관계가 약해 보이나, N=1 노이즈일 가능성을 배제 못 함 → **`--repeat 3` 확정측정 진행 중**(bong1/ytn2/sbs1). median·max로 재판정 예정.
