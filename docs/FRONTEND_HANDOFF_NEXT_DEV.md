...
### 4-5. [중간] 세션 언어 선택 UI + `?language=` 전송
- **현황**: 백엔드는 WS 쿼리 `?language=auto|ko|en`를 수용하나 프론트가 안 보내 항상 `auto`로 동작. 언어 선택
  UI가 없다.
- **요구사항**: 설정에 언어(자동/한국어/영어) 셀렉터 추가 + 연결 시 `?language=${선택값}` 부착. 세션 도중 변경은
  재연결을 의미하므로 **연결 전에만 변경 가능**하게 권장. 상세 스펙: [FRONTEND_LANGUAGE_SELECT_PATCH.md](FRONTEND_LANGUAGE_SELECT_PATCH.md).
...
- **현재 배포 dist의 소스 전체 접근이 필요합니다.** 백엔드팀은 진단을 위해 dist 소스 일부(store·hooks·api·
  일부 컴포넌트)만 텍스트로 전달받았고, **관리자 이동 버튼·녹음 컨트롤(재개/종료) 컴포넌트·라우터
  (`router.tsx`/`routeTree.gen.ts`)·관리자 페이지 컴포넌트**는 확보하지 못했습니다. §4를 정확히 수정하려면 해당
  소스가 필요합니다.
