...
> **사후 갱신에도 신규 메시지 타입은 없다.** 녹음 중 관리자 페이지에서 단어 대치 사전을 바꾸면 서버가
> 이미 내보낸 과거 줄의 `text`(그리고 재번역된 `translation`)를 고쳐 다시 보내는데, 이는 전적으로 위
> `new_lines` 꼬리 교체 규약으로 표현된다(공통 prefix가 그 줄에서 깨지므로 그 뒤가 통째로 재전송된다).
