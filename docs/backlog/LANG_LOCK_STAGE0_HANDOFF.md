...
>
> **⛔ 정지 사유 (먼저 읽을 것)**: 단일언어 ko 세션에서 `p_opp≥0.97` 발동이 **5건**(순수 오탐 2건 확정)
> 관측돼, 안 A 착수 시 사전 등록한 진행 조건("단일언어 세션 정상 전사 구간 발동 0건")을 충족하지 못했다.
> 단 발동 5건은 전부 K=1 단발·`seglen` 0.24~1.02s로 **Stage 1 게이트에 전부 걸린다**(would_fire 0).
> 수치·전수 분류·양쪽 근거 = **§4-0**. 상세 정본 = `docs/research/SOT_LANG_PROBE_STAGE0.md` §10.
...
## 4. 다음 단계 — 안 A 완료, 안 B는 **정지 상태**(사용자 판단 대기)
...
| **B. Stage 1 섀도우 판정** | 게이트 G1~G9(최소 버퍼길이·연속 K배치·쿨다운·다른 트리거 중복방지·auto 가드) 구현하되 **적용 안 함**, `would_fire`만 로깅. 전 테스트셋 측정 | ⛔ **미착수 — 사전 등록 분기 기준 미충족으로 정지**(§4-0). 사용자 판단 필요 |
| **C. 종료** | `/log-experiment`로 Exp 번호 부여·기록하고 닫는다. 계측·지표는 브랜치에 남아 언제든 재개 가능 | Exp-195 부여·기록 완료. 종료 여부는 미결정 |
...
**재개한다면 권고 순서**: ① kor1~3 `--repeat 3` 재측정으로 오탐률을 점추정으로 승격 →
② 그때도 게이트 통과 0이면 안 B(섀도우) 진행. ①을 건너뛰고 ②로 가면 근거가 1회 표본에 얹힌다.
...
- ~~**단일언어 세션(kor1~3, eng1) 오탐률 전혀 모름**~~ → **해소(Exp-195, §4-0)**. 답: ko 세션에서
  ...
  다만 `--repeat 1` 1회 표본이라 **오탐률의 점추정은 아직 없다** — N≥3 재측정이 남은 과제.
