...
> 정본 = `docs/research/SOT_LANG_PROBE_STAGE0.md` §12. 브랜치 `exp/lang-lock-stage1-duration`
> (`6868c2d`, master 미머지).

§4-0-3 사후분석에서 **T=1.0s가 bong1의 확정 참양성 하나를 0.04s 차로 놓치고 있었다**는 것이
드러나 `STAGE1_MIN_DURATION`을 **1.0 → 0.8**로 완화했다. 상수 1개 + 회귀 테스트 3건이 전부이고
게이트 로직·섀도우 불변식은 무변경(`_apply_detected_language` 신규 호출 0건 유지, pytest
674→677 pass·1 skip, ruff clean).

- **놓친 참양성**: 정답 `bong1.txt:43`(`아니 그 (더듬) 플라스틱 말랑말랑한 것도 만들었죠…`)이
  한국어로 시작했다가 `Plastic, sorry` / `Malang mal` / `ang an` / `awesome thing to make`로
  뒤집힌 확정 언어잠금 실패. 버스트 t=91.20(k=1)→92.16(k=4, p_opp=0.9996, seglen=5.32,
  **dur=0.96**)이 G3/G4/K≥3을 다 통과하고 **T에서만** 막혔다(§11-5 bong1 `blocked_g5=5`의 실체).
- **주 증거는 오프라인 리플레이**: 게이트가 프로브 스트림의 순수 함수라 로그만으로 임의 상수의
  `would_fire`를 재계산할 수 있다. Exp-197 4파일의 실측 `[Stage1ShadowStats]` 카운트를 **정확히
  재현**해 검증했다(함정: `infer(is_last=True)`가 그 호출의 마지막 프로브를 처리하기 *전에* 요약을
  방출 → 마지막 프로브 1개를 빼야 일치. §11-2의 중복억제 버그와는 별개 사안).
- **T 스윕**(bong1/ytn2/sbs1/ytn1): 1.0 → 0/0/0/15 · 0.9 → 1/0/0/16 · **0.8 → 1/0/0/16** ·
  0.7 → 2/0/0/17 · 0.5 → 2/0/0/17 · 0.0 → 2/0/0/19. **에피소드 커버리지가 T=0.5~0.9에서 동일**
  (달라지는 건 에피소드 내 발동 지연뿐) → 0.8은 knife-edge가 아니다. ytn2·sbs1은 T=0.0에서도 0.
- **오탐 위험 0**: ko/en 16세션 로그(kor1~3×4 + eng1×4)를 **G1 우회**(= 같은 단일언어 내용이
  auto 세션에 들어온 worst case)로 리플레이 → 모든 T(0.0 포함)에서 `would_fire`=0. `p_opp≥0.97`
  통과 프로브의 최대 seglen이 **1.02s**(G3 문턱 2.0s, 여유 0.98s) — **차단은 전적으로 G3의 몫이고
  T의 기여분은 0**이다. 즉 T 완화는 오탐 방어를 잠식하지 않는다.
- **실측(경로 C, `--lan auto --repeat 1`)**: bong1 24.7/62.5/23.5(fire 0) · sbs1 8.9/80.0/90.0
  (fire 0) · ytn1 11.7/84.2/57.1(fire **2**) · ytn2는 1차 run 폐기(WER 69.0/화자F1 26.7 —
  `FFmpeg read timeout`·lag 58.70s·프로브 150으로 반토막 = 하니스/자원 stall, 로깅 전용 diff라
  디코딩 영향 불가) 후 한산한 머신에서 N=2 재측정 → **median WER 11.1% / 화자F1 95.0%**
  (8.4/13.8, stdev 3.8, lag ≤0.99s, timeout 0, fire 양쪽 0). **Exp-197 수치와의 차이는 전부
  회차간 분산이며 개선/회귀가 아니다**(diff가 로깅 전용).

**새로 생긴 주의사항 — `would_fire`의 실현편차**: 실측 run은 bong1 발동을 재현하지 못했고
(`would_fire=0`, `blocked_g5=2`, 다른 오디오 실현이라 같은 버스트가 안 생김), ytn1은 15건 → 2건으로
줄었다. **게이트 로직 질문의 주 증거는 결정론적 리플레이**이고 실측 run의 역할은 end-to-end 확인
(구현 동작·로그 정상·회귀 없음 — 달성)이다.

**지금 남은 결정은 §4-0-3과 동일**(섀도우 → 실제 트리거 배선, 사용자 판단 대기). 여기에 **판단 조건이
하나 늘었다**: 게이트 발동은 회차마다 있다가 없다가 하는 희소·간헐 이벤트이므로, 승격하더라도 효과가
회차별로 나타났다 사라진다 — **단회 측정으로 이익/해악을 귀속할 수 없고** N≥3 반복 + 발동 카운트
기반 짝지음 대조가 사실상 필수다.
