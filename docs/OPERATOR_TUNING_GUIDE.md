...
> **"권장 `--lan`" 열은 배포 운영 권장값이다.** 성능 측정(채택/기각)은 데이터·상황과 무관하게 항상
> `--lan auto`로 수행한다(CLAUDE.md §3.2·§4) — 이 열을 측정 지시로 읽지 말 것.
...
**세션 언어모드(`--lan`)는 프리셋과 별개로 직교하게 지정한다** — §3의 "권장 `--lan`" 열 참조(배포 운영
권장; 성능 측정은 항상 `--lan auto`). 예를 들어
