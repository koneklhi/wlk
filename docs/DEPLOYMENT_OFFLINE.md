...
| `--interim-hangul-weight` | 2.8 | 한글 1자를 라틴 몇 자로 셀지. **올리면 한국어만 더 빨리** 발동한다(라틴 전용 텍스트는 영향 없음) |
...
> 기본값에서 첫 발동은 라틴 12자 / 한글 약 4.3자이며, 실측 발화 속도로는 양쪽 다 약 0.8~0.95초다.
...
| **한국어를 말할 때만** 첫 영어 번역이 문장 절반쯤 지나서야 뜸 | 미리보기 번역 발동 게이트를 raw 문자수로 재던 시절의 증상. 한글은 라틴보다 정보 밀도가 높아(실측 한국어 5.5자/초 vs 영어 15.3자/초) 같은 12자 임계에 **2.8배 늦게** 도달했다 — 한국어 2.2초 / 영어 0.8초 | 해소됨. 게이트 단위를 `effective_len`(한글 2.8배 가중)으로 바꿔 양방향 첫 미리보기가 발화 약 0.8~0.95초에 대칭으로 뜬다. 요청률 상한(0.5초 디바운스·in-flight 1개)은 그대로라 LLM 부하는 증가하지 않는다 |
...
| 번역이 잠깐 **여러 줄**로 떴다가 정상 번역으로 덮여 사라짐 | 미확정 버퍼의 짧은 조각을 gpt-oss-20b가 문장으로 상상(환각) | `_sanitize_result()`가 개행 시 첫 줄만 취하고 500자 초과 시 절단, 미확정 버퍼가 6 effective자 미만이면 번역 보류로 완화됨(단위는 raw 문자수가 아니라 `effective_len` — 한글 가중 2.8배)(확정 경로 무한 재번역 방지는 manager의 시도 상한이 담당). 서버 로그 `[Translation] 초장문 출력 절단` |
