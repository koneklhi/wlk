...
| 번역이 원문과 **같은 언어**로 그대로 나옴 | STT `detected_language` 오검출로 번역 방향이 반대로 잡혀 LLM이 이미 목표 언어인 입력을 되돌려줌 | 서버 로그 `[Translation] detected_language=… 문자구성 추정=… 보정` 확인. `resolve_src_lang()`이 텍스트 문자구성(한글/영문 ≥85%)이 명확할 때만 방향을 자동 보정하고, 한·영 혼합문(code-switching)은 STT 판단을 존중해 보류하므로 혼합문에서는 여전히 나올 수 있음 |
| 번역이 잠깐 **여러 줄**로 떴다가 정상 번역으로 덮여 사라짐 | 미확정 버퍼의 짧은 조각을 gpt-oss-20b가 문장으로 상상(환각) | `_sanitize_result()`가 개행 시 첫 줄만 취하고 500자 초과 시 절단(폐기 아님 — 폐기하면 확정 경로가 매 tick 재번역), 미확정 버퍼 6자 미만은 번역 보류로 완화됨. 서버 로그 `[Translation] 초장문 출력 절단` |
