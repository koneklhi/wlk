...
**미리보기(interim) 번역 튜닝 노브 — 배포 PC에서 코드 수정 없이 조정**: 미확정 버퍼 미리보기 번역이
언제 발동할지, 에코를 어떻게 처리할지를 CLI로 바꿀 수 있다. 전부 **미지정이면 코드 상수 폴백**(무회귀).

| 플래그 | 기본 | 무엇을 바꾸나 |
|---|---|---|
| `--interim-min-delta-chars` | 12 | **첫 미리보기 발동 임계.** 새 줄에서는 직전 소스가 비어 있어 이 값이 곧 첫 발동 기준이다 — **번역을 앞당기려면 이 값을 줄인다** |
| `--interim-min-chars` | 6 | 이 길이 미만이면 아예 요청하지 않는 하한. 첫 발동은 위 델타 게이트가 더 엄격해서 보통 이 값이 아니라 델타가 지배한다 |
| `--interim-hangul-weight` | 4.0 | 한글 1자를 라틴 몇 자로 셀지. **올리면 한국어만 더 빨리** 발동한다(라틴 전용 텍스트는 영향 없음). 4.0은 대칭점(2.8)보다 한국어를 의도적으로 더 빠르게 둔 값(2026-08-20 배포 요청) |
| `--interim-min-interval` | 0.5 | 발동 최소 간격(초) = LLM 요청률 상한 |
| `--interim-echo-policy` | `retry` | 결과가 원문과 같은 언어일 때: `retry`(방향 지시문 재시도) / `discard`(폐기) / `off`(게이트 미적용) |
| `--interim-strict-direction` | ON | 첫 호출부터 출력 언어 강제 지시문을 붙여 에코를 **예방**(LLM 왕복 증가 없음) |

> **길이 단위는 raw 문자수가 아니라 `effective_len`이다** — 한글 1자 = `--interim-hangul-weight` 자.
> 기본값(4.0)에서 첫 발동은 라틴 12자 / 한글 3자이며, 실측 발화 속도로는 한국어 약 0.55~0.75초 /
> 영어 약 0.8초다 — **한국어가 영어보다 빠르다**(대칭이던 2.8에서 의도적으로 한 단계 더 당김).

배포 PC 스윕 예시(한국어를 더 빨리 띄우고 싶을 때 — 한 번에 하나씩만 바꿔 비교):

```powershell
# A. 기본 (비교 기준)
python -m whisperlivekit.basic_server --lan auto --diarization ...

# B. 첫 발동을 절반으로 — 양 언어 모두 빨라진다
... --interim-min-delta-chars 6

# C. 한국어만 더 빨리 — 영어는 그대로
... --interim-hangul-weight 5.0    # 기본(4.0)보다 더 당기고 싶을 때

# D. 에코 폐기가 병목인지 확인 (로그에 '에코 감지(interim)'가 잦을 때)
... --interim-echo-policy off        # 위양성 진단용: 폐기 없이 결과를 그대로 표시
... --no-interim-strict-direction    # 선제 지시문을 끄고 에코 빈도 비교
```

> **주의 — 무작정 낮추는 게 답이 아니다.** 임계를 낮출수록 LLM에 넘어가는 조각이 짧아지고, 짧은 조각일수록
> gpt-oss-20b가 번역하지 못하고 원문을 되돌려주는(에코) 빈도가 올라간다. 에코가 나면 그 결과는 버려지므로
> **발동은 빨라졌는데 화면에 뜨는 건 오히려 늦어질 수 있다.** 서버 로그의 `에코 감지(interim)` 빈도를
> 함께 보면서 조정할 것. 요청률(`--interim-min-interval`)을 함께 낮추면 단일 llama.cpp 서버가 밀려
> 확정 번역까지 지연될 수 있다.
...
| 서버 로그에 `에코 감지(interim) — 재시도 없이 폐기`가 **다발** | 짧은 미확정 조각을 gpt-oss-20b가 번역하지 못하고 원문(한국어)을 되돌려주는 현상. 과거엔 그 결과를 버리고 **재시도하지 않았고**, `_interim_source`는 이미 그 텍스트로 전진해 있어 **버퍼가 델타 임계만큼 더 자랄 때까지 같은 텍스트를 다시 시도조차 하지 않았다** → 미리보기가 멎어 보인다. 게이트를 앞당길수록(조각이 짧아질수록) 더 잦아진다 | ① `--interim-strict-direction`(기본 ON)으로 **첫 호출부터** 방향 지시문을 붙여 에코를 예방(왕복 증가 없음) ② `--interim-echo-policy retry`(기본)로 에코 시 1회 재시도 ③ 로그의 `result=`가 멀쩡한 영어인데 폐기된다면 `_is_echo` 위양성이므로 `--interim-echo-policy off`로 확인 |
| **한국어를 말할 때만** 첫 영어 번역이 문장 절반쯤 지나서야 뜸 | 미리보기 번역 발동 게이트를 raw 문자수로 재던 시절의 증상. 한글은 라틴보다 정보 밀도가 높아(실측 한국어 5.5자/초 vs 영어 15.3자/초) 같은 12자 임계에 **2.8배 늦게** 도달했다 — 한국어 2.2초 / 영어 0.8초 | 해소됨. 게이트 단위를 `effective_len`(한글 가중)으로 바꿔 첫 미리보기가 앞당겨진다. 기본 가중치는 4.0 — 대칭점(2.8, 약 0.9초 양쪽 동일)보다 한 단계 더 당겨 한국어(약 0.55~0.75초)가 영어(약 0.8초)보다 빠르다(2026-08-20 배포 요청). 요청률 상한(0.5초 디바운스·in-flight 1개)은 그대로라 LLM 부하는 증가하지 않는다 |
| 확정된 줄의 번역칸이 잠깐 **빈칸**이 됨(로더도 없이) | 프론트 로더 조건이 `!finalized` 뿐이라 확정 순간 '번역 중…'이 사라졌다 | 해소됨. 서버가 `lines[].translation_pending`으로 대기 상태를 알려주고 프론트가 확정 이후에도 로더를 유지한다. 번역이 빈값으로 정착한 줄에는 플래그가 오지 않아 스피너가 영구히 남지 않는다 |
...
| 번역이 원문과 **같은 언어**로 그대로 나옴 | STT `detected_language` 오검출/캐리오버로 번역 방향이 반대로 잡혀 LLM이 이미 목표 언어인 입력을 되돌려줌(에코) | 3중 방어로 완화됨: ① 입력측 `resolve_src_lang()` — 순수 스크립트는 즉시 확정(한글 1자·라틴 4자 이상), 혼합문은 다수결(우세 60%)로 보정, ② 출력측 에코 게이트 — 확정 경로에서 결과가 원문과 같은 언어면 src 강제 + 방향 지시문으로 1회 재시도(서버 로그 `[Translation] 에코 감지`), ③ 재시도까지 에코면 빈 번역(`""`)으로 캐시에 정착해 무한 재번역을 끊음(로그 `확정 번역 N회 연속 실패`). 미확정(interim) 경로도 이제 같은 재시도를 한다(`--interim-echo-policy`, 기본 `retry`) — 과거 `discard` 고정이던 것이 미리보기 지연의 실제 병목이었다. 한·영 근소차 혼합문은 판정 보류(STT 존중)라 이론상 잔존 가능 |
| 번역이 잠깐 **여러 줄**로 떴다가 정상 번역으로 덮여 사라짐 | 미확정 버퍼의 짧은 조각을 gpt-oss-20b가 문장으로 상상(환각) | `_sanitize_result()`가 개행 시 첫 줄만 취하고 500자 초과 시 절단, 미확정 버퍼가 6 effective자 미만이면 번역 보류로 완화됨(단위는 raw 문자수가 아니라 `effective_len` — 한글 가중 기본 4.0)(확정 경로 무한 재번역 방지는 manager의 시도 상한이 담당). 서버 로그 `[Translation] 초장문 출력 절단` |
