...
| 번역이 원문과 **같은 언어**로 그대로 나옴 | STT `detected_language` 오검출/캐리오버로 번역 방향이 반대로 잡혀 LLM이 이미 목표 언어인 입력을 되돌려줌(에코) | 3중 방어로 완화됨: ① 입력측 `resolve_src_lang()` — 순수 스크립트는 즉시 확정(한글 1자·라틴 4자 이상), 혼합문은 다수결(우세 60%)로 보정, ② 출력측 에코 게이트 — 확정 경로에서 결과가 원문과 같은 언어면 src 강제 + 방향 지시문으로 1회 재시도(서버 로그 `[Translation] 에코 감지`), ③ 재시도까지 에코면 빈 번역(`""`)으로 캐시에 정착해 무한 재번역을 끊음(로그 `확정 번역 N회 연속 실패`). 미확정(interim) 경로는 재시도 없이 즉시 폐기. 한·영 근소차 혼합문은 판정 보류(STT 존중)라 이론상 잔존 가능 |
...
| 트랩 | 증상 | 조치 |
|---|---|---|
| **배포 PC 브라우저 캐시로 프론트 변경이 반영 안 됨 (2026-08-20)** | 반입·적용을 마쳤는데 "성능/동작에 변화가 없다"로 보인다. 백엔드는 새 코드로 도는데 브라우저가 캐시된 `index.html`을 들고 있어 **구버전 해시 asset(`index-<old>.js`)을 계속 로드**하기 때문. 백엔드와 프론트가 **함께** 바뀐 변경에서는 프론트만 구버전이어도 전체가 "안 고쳐진 것"처럼 보인다 | 반입 후 첫 확인은 **강제 새로고침(Ctrl+F5) 또는 시크릿 창**으로 한다. 그래도 의심되면 DevTools Network 탭의 로드된 `index-<hash>.js`가 `frontend\static\index.html`이 참조하는 해시와 같은지 대조한다. **"변화 없음"을 코드 문제로 진단하기 전에 이것부터 배제할 것** — 실제로 이 캐시 때문에 정상 동작하던 변경(한→영 미리보기 번역 개선)을 "효과 없음"으로 오판해 불필요한 후속 작업 2회차를 진행했고 전량 롤백했다(구현은 `archive/interim-echo-and-knobs` 브랜치에 보존) |
