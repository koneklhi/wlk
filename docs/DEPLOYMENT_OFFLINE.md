...
| 번역이 잠깐 **여러 줄**로 떴다가 정상 번역으로 덮여 사라짐 | 미확정 버퍼의 짧은 조각을 gpt-oss-20b가 문장으로 상상(환각) | `_sanitize_result()`가 개행 시 첫 줄만 취하고 500자 초과 시 절단, 미확정 버퍼 6자 미만은 번역 보류로 완화됨(확정 경로 무한 재번역 방지는 manager의 시도 상한이 담당). 서버 로그 `[Translation] 초장문 출력 절단` |
