...
미리보기 번역의 발동 임계는 **문자 수가 아니라 `effective_len`(한글 가중 2.8배) 기준**이다
(`translator.effective_len`). raw 문자 수로 재면 한글이 라틴보다 정보 밀도가 높아 같은 임계에 2.8배 늦게
도달하고, 그 결과 **한→영 미리보기 번역만 늦게 뜬다**(실측: 한국어 2.2초 vs 영어 0.8초). effective 기준으로
재면 양방향 첫 미리보기가 발화 약 0.8~0.95초에서 대칭으로 뜬다.
