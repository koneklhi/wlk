...
**목표 필수 기능 예외 (자율 기각 금지 → 사용자 질의)**: 위 1~5 정량 게이트 미달이라도, 그 변경이 §3.1 폐쇄망·§3.2 한/영 두 언어 고정 등 핵심 불변 제약 달성에 필요한 기반 기능이면 자율 기각하지 않는다. ⓐ목표·제약 근거 ⓑ회귀 위치(worst-case 포함) ⓒ대안 구현 여지를 함께 보고하고 **사용자에게 채택 여부를 묻는다**. 일반 WER/F1 개선 실험에는 해당 없음.
