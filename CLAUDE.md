...
- **범용 개선만**: primary 측정 데이터(sbs1/ytn1/eng1)에 과적합되는 개선 금지. 한국어·영어 범용 전사 품질이 목표.
  추후 테스트 데이터 추가 시 회귀 없어야 한다. 새 실험 전 "이 변경이 sbs1/ytn1 특화인가?" 자문.
  **ytn2는 held-out** — primary 기준 모두 통과한 채택 후보만 단회(`--repeat 1`) 측정해 코드스위칭 일반화 검증.
  평소 `--repeat 3` 루틴에 ytn2 포함 금지 (일반화 데이터로서의 가치 보존).
...
- **최신 수치**: 경로 C eval.py 측정 결과 (sbs1/ytn1/eng1 WER, F1, 측정 회차)
