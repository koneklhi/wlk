...
- 단어 교정 사전 + 번역 glossary를 운용 중 **동적으로 추가/삭제**할 수 있다(REST `/api/corrections`·`/api/prompts`). 사전 갱신은 **즉시 반영**되며 **녹음(전사 세션) 진행 중에도 소급 적용**된다:
  - **전사 텍스트 대치 = 세션 전 구간 소급.** 서버가 매 tick(~50ms) 세션 전체 `lines[]`에 사전을 다시 적용하므로 추가 비용이 사실상 없다. 미확정 버퍼도 포함하며, 항목을 지우면 원문으로 **원복**된다.
  - **번역 = 최근 N개 확정 문장만 소급 재번역**(`--retro-retranslate-lines`, 기본 20 / `0`이면 비활성). 재번역은 LLM 호출이라 전 구간을 소급하면 단일 번역 서버가 밀린다 — **최초** 번역은 이 창과 무관하게 항상 수행된다. 상세·한계(일시중단 이전 구간 제외, 멀티워커 미지원) = [docs/API_SPEC.md](docs/API_SPEC.md) §3.5.
